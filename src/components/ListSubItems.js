  ...
  return (
    <>
      ...
      <ListItem
        ...
        dense
        button
        onClick={context.handleSubTopic(props.value)}
        className={
          context.selectedSubtopic === props.value ? classes.listSelected : null
        }
        selected={context.selectedSubtopic === props.value}
      >
        <ListItemIcon className={classes.listItem}>
          {contextSide.open && (
            <>
              <Videocam fontSize="small" />
              <ListItemText primary={props.title} className="ml-1" />
            </>
          )}
          <div></div>
          <FormControlLabel
